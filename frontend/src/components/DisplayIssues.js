...
function DisplayIssues({ userData, isLoggedIn, complaints, setComplaints }) {
            ...
            <thead className="table-dark">
            ...
            </thead>
              {
                issue.map((row) => (
                 ...
                 <td>{row.status}</td>
                  <td>
                  {/* <button  className="btn btn-primary me-4" onClick={handleEdit(row)}>Update</button> */}
                  <button
                    className="btn btn-primary me-4"
                    onClick={() => {
                      if (editingId === row.issueId) {
                        handleSave();
                        console.log(row.issueDescription+"save")
                      } else {
                        handleEdit(row);
                        console.log(row.issueDescription+"edit")
                      }
                    }}
                  >
                  ...
                  <button className="btn btn-danger" onClick={()=>{
                    handleDelete(row.issueId)}}>Delete</button>
                  </td>
                  </tr>
                  </tbody>
