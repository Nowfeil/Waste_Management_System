...
async function login(req, res) {
    ...
    try {
        ...
        if (user && user.password==password) {
            return res.status(400).json({ message: `User with ${email} logged in successfully.` });
        }
    } catch (err) {
...
async function signup(req, res) {
    ...
    try {
        const user = await loginModel.findOne({ email });
        if(user && user.password==password){
          return res.status(400).json({ message: `User with ${email} already has an account. Please sign in.` });
        }
        else{
